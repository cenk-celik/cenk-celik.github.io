...
Citation counts come from Google Scholar first, with an automatic fallback to the [Semantic Scholar Academic Graph API](https://api.semanticscholar.org/) on any failure.

Google Scholar has no official API, and blocks or CAPTCHAs almost every automated request. Two earlier approaches were tried and dropped: scraping it directly (blocked on essentially every GitHub Actions run - shared runner IPs are heavily flagged), and routing through a paid/registered proxy or scraping-API service (ScraperAPI, Webshare, SerpApi in turn), each of which either turned out not to be free in practice or required registering with a third-party website - ruled out by design here, so no external account of any kind is used. The script instead uses the `scholarly` library with `ProxyGenerator.FreeProxies()` - a free, automatically-sourced rotating proxy that needs no signup at all. That comes at a real cost: free proxies are frequently slow, already dead, or already blocked by Google themselves, so this fails more often than a paid proxy would have. That's the accepted trade-off for zero registration anywhere, not a bug. When it does fail, the script automatically falls back to the [Semantic Scholar Academic Graph API](https://api.semanticscholar.org/) instead of giving up - a plain JSON API, no key or signup needed, and hasn't been blocked once in testing. Numbers from Semantic Scholar typically read a little lower than Google Scholar's for the same paper, since the two index different sources — that's a coverage difference, not a bug.

Either way, each entry in `publications.json` is matched by DOI first (Semantic Scholar fallback only - Google Scholar doesn't expose a clean DOI), falling back to normalised title, and only its `citations` field is updated — title, authors, venue, type, links and `selected` stay exactly as hand-curated. New publications are **not** auto-added from either source (both occasionally split one real paper into two records - e.g. a preprint indexed separately from its published version - which makes auto-adding risky), so add new entries by hand; the next sync then keeps their citation count current. The script is still **keep-last-good** end to end: if both sources fail, it logs a warning and exits without touching the data files.
